...
describe('@lantean/workspace devkit addNxNamedInput', () => {
  ...
  it('should add the implicit dependency', () => {
    ...
    expect(
      readJson<NxJsonConfiguration>(tree, nxConfigFile).namedInputs?.example
    ).toStrictEqual(exampleNamedInput.example);
  });
  ...
  it('should add the implicit dependency as default if there where none', () => {
    ...
    expect(tree.exists(nxConfigFile)).toBeTruthy();
    expect(
      readJson<NxJsonConfiguration>(tree, nxConfigFile).namedInputs
    ).toStrictEqual({
      ...exampleNamedInput,
      default: ['example'],
    });
