...
describe('@lantean/workspace codecov', () => {
  let tree: Tree;
  ...
  describe('update codecov projects', () => {
    ...
    it('should configure the project, jest configuration and codecov', () => {
      ...
      expect(
        tree.read(jestConfigPath)?.includes(`"coverageReporters": ["lcov"]`)
      ).toBeTruthy();
      expect(readCodecov(tree).flags).toStrictEqual({
        lib1: { paths: ['libs/lib1'] },
      });
      expect(readCodecov(tree).coverage?.status.project.lib1).toStrictEqual({
        flags: ['lib1'],
      });
    });
    ...
    it('should ignore non testable projects', () => {
      ...
      addProjectConfiguration(tree, 'lib5', {
        ...
        targets: { test: { command: 'npm run test' } },
      });
    ...
    it('should just create codecov file if no projects exist', () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        `There are no projects that can be configured with codecov.`
      );
    ...
    it('should not overwrite coverage reporters if already existing', () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        `Coverage reporters already configured for file: ${jestConfigPath}`
      );
    });
    ...
    addProjectConfiguration(tree, 'lib1', {
      ...
      targets: {
        test: {
          executor: jestExecutor,
          outputs: ['coverage/libs/lib1'],
          options: { jestConfig: jestConfigPath, passWithNoTests: true },
        },
      },
    });
    tree.write(
