...
export function createCodecov(tree: Tree) {
  ...
  if (tree.exists(codecovFile)) return;
...
export function writeProjectsToCodecov(tree: Tree): void {
  ...
  projects.forEach(
    (projectConfiguration: ProjectConfiguration, projectName: string) => {
      ...
      if (testTarget != null && testTarget.executor?.includes('jest')) {
        ...
        updateProjectJestCoverage(
          ...
          testTarget.options.jestConfig ??
            `${projectConfiguration.root}/jest.config.ts`
        );
      } else {
  ...
  if (ignoredProjects.length > 0) {
    ...
    console.log(
      `Ignored projects where a test target with a jest executor was not detected: ${ignoredProjectsList}`
    );
  }
...
export function updateProjectJestCoverage(tree: Tree, jestConfigPath: string) {
  const jestFileContent: string | undefined = tree
    .read(jestConfigPath)
    ?.toString();
